...
NetworkOps AI Agent v0.5.0-alpha keeps three compatible orchestration generations:
...
The Streamlit console keeps the original chat and evidence view while adding incident creation, approval, Trace, Timeline, metrics, and benchmark tabs. SQLite remains a single-process demonstration backend; OpenTelemetry, OTLP export, centralized time-series storage, authentication, and retention policies are not implemented.
...
The included Docker Compose starts PostgreSQL and Redis for local development only. It does not contain an application container, production credentials, automatic failover, cross-backend replication, or a storage-health API.
